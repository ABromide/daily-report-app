...
- 阅读节奏：长文不要写成连续文本墙。单个自然段尽量控制在 180 到 220 个中文字符以内；当一个小节包含多个层次时，优先拆成 h3 小标题、问题式小节、列表、表格、公式解释块或 Mermaid 图。连续三段以上的纯文本说明通常应重写成结构化展示。
- 方法公式与算法步骤：尽可能保留原文公式、目标函数、奖励函数、指标定义、算法伪代码和变量解释；如果原文公式较少，可以补充解释性公式，但必须说明它是为了辅助理解。
- 本地附件图片、Figure 与 Table 解读：尽可能引用原始博客图片、论文 figure/table、项目页图片、系统图、轨迹图、结果图或 GitHub README 架构图；但用于 Markdown 展示的图片必须先下载到本地 `public/assets/`，再随发布工具上传为附件。
- 实验设置、主结果与消融：按数据集、基座模型、训练设置、指标、主结果、消融和失败案例逐项展开。
- 细节清单：尽可能写出模块名、数据规模、训练阶段、benchmark、baseline、指标数字、消融项、失败类型、figure/table 编号、原图 URL 和本地附件路径。
...
- 6. Figure/Table 逐图逐表解读：尽可能引用已下载并上传的本地附件图，解释每张图和每张表的证据功能。
...
- 链接、已上传的本地附件图片、公式、表格和 Mermaid 图直接保留在 Markdown 中。
...
如果原文有关键图片、表格、系统图或结果图，自动化必须先把要展示的图片下载到 data worktree 的 `public/assets/YYYY/MM/DD/ITEM_ID/`，再随 `publish-public-run` 提交到 data 分支，相当于附件上传。Markdown 图片语法只引用上传后的本地附件路径，例如 `![图 1：系统架构](assets/YYYY/MM/DD/ITEM_ID/figure-1.png)`；不要直接把外部图片 URL 放进 Markdown 图片语法。如果图片无法下载或格式不适合镜像，只能把原图页面作为普通来源链接，并说明未能本地化的原因。图片最好配中文说明：这张图对应哪一节、展示什么证据、能支持什么结论、不能证明什么。除非原文确实没有图片或 PDF 图片不可提取，否则每篇尽量包含 1 张关键图。
...
- 自动化建议尽量抓取外部原始图片、论文 figure/table、项目页图、轨迹图和截图；凡是要在 Markdown 中展示的图片，都必须先下载到本地 `public/assets/`，再随发布工具上传为附件
...
- 自动化写作要尽可能挖掘方法模块、数据规模、训练设置、benchmark、baseline、消融项、失败案例、图表编号、原图 URL、本地附件路径和公式变量
