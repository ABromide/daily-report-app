...
每篇文章生成一个独立 Markdown 文件，路径写入 `analysis_markdown_path`。这个文件不写完整 HTML，不内联 CSS，不做复杂结构校验；前端只负责把 Markdown 渲染到 Daily Report 的 parchment / ink-blue 阅读框里。Markdown 可以直接包含普通链接、图片链接、行内公式 `$...$` 和块级公式 `$$...$$`。正文目标是尽量达到 5000 中文字以上，标题、URL、JSON 字段和代码块不计入主要分析字数。

Markdown 分析稿可以参考 [MLPod 论文解读长文写法](https://www.mlpod.com/1548.html)：开头给论文或项目元信息、原始链接和核心图；`TL;DR` 可以写成长摘要；正文可以用编号章节推进，像长文解读一样逐段拆开问题、方法、证据、图表、讨论和局限。这个参考不是固定模板，模型应根据原文自然组织 Markdown 标题；论文、博客/报告、代码项目不能互相套模板。

所有类型都可以优先考虑这些模块，但不要求逐字逐项填满：

- 论文或项目元信息：标题、来源、发布时间、原始链接、论文 PDF、代码仓库、作者页。
- TL;DR：长摘要，建议交代问题、方法、关键结论、实际意义和局限。
...
- 论文、博客/报告、代码项目应使用不同正文结构；博客不能硬套论文实验模板
- 自动化建议尽量抓取原始图片、论文 figure/table，并搜索第三方解读、相关论文和作者补充材料
