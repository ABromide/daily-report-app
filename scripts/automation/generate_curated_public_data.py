            ...
            "article": {
                ...
                "parts": [
                    ...
                    },
                    {
                        ...
                        "body": "GitHub release 显示 v0.11.6 发布于 2026-05-29，主要变化包括 tracing span lifecycle dispatch helpers，以及 streaming/chat completions 的 generation span model metadata 修复。v0.11.5 发布于 2026-05-22，变化更多：可配置 tracing ID、scoped trace context helpers、resumed runs 的 RunState trace clearing、completed tracing lifecycle dispatch helpers、Usage JSON restoration helpers 等。这些变化不等于 SDK 已经在所有生产场景成熟，但它们很明确地说明维护者正在补运行诊断、恢复、上下文传播和可观测性这些工程骨架。",
                    },
                    {
                        "title": "6 月 5 日 commit：不是功能 release，而是文档翻译",
                        "body": "GitHub commits API 显示 2026-06-05 的最新 commit 是 `docs: translate pages`。所以这个 item 的日期窗口可以成立，但不能把它写成“6 月 5 日发布了重大功能”。准确说法应该是：本周内仓库仍有维护活动，最新 commit 是文档翻译；最近一次功能 release 是 5 月 29 日 v0.11.6，功能主线与 tracing 相关。",
                    },
                ],
                "flow": [
                    {"title": "先定义 Agent", "body": "用 instructions、tools、guardrails 和 handoffs 描述角色边界，让 Agent 不只是一个 prompt，而是一个可组合的执行单元。"},
                    {"title": "再选择执行面", "body": "普通 Agent 覆盖轻量请求；Sandbox Agent 在需要文件系统、仓库内容、命令执行和长任务状态时启用。"},
                    {"title": "把状态和人类审批接进去", "body": "Sessions 管理跨运行历史，Human in the loop 处理需要人工确认的关键节点，Guardrails 负责输入输出约束。"},
                    {"title": "最后用 tracing 看清运行过程", "body": "Tracing 记录 agent runs，release notes 中的 trace ID、trace context、lifecycle dispatch helpers 和 usage restoration 都指向这一层。"},
                ],
