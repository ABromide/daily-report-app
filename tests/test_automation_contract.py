...
def test_chinese_automation_contract_is_recent_deduped_and_categorized() -> None:
    ...
    assert "背景与研究问题" in contract["analysis_requirements"]["paper_rule"]
    assert "figures/tables" in contract["analysis_requirements"]["paper_rule"]
    assert "尽可能保留原论文公式" in contract["analysis_requirements"]["paper_rule"]
    ...
    assert "论文必须按这个正文顺序写" in prompt_text
