...
def test_sample_manifest_contains_article_markdown_and_audit_record(tmp_path: Path) -> None:
    ...
    assert "来源与材料地图" not in article_markdown
    assert "来源与本地附件" not in article_markdown
    assert "Markdown 排版要求" in article_markdown
    assert "内容类型分析建议" in article_markdown
    assert "领域延伸思考" in article_markdown
    assert "研究者结论" in article_markdown
